...
<head>
    ...
    <title>Smart Ingeniería — Editar Trabajador</title>
    <style>
...
<div class="main">
    <div class="topbar">
        <div>
            <p style="color:#fff;font-weight:700;font-size:15px">Editar Trabajador</p>
            <p style="color:#52525b;font-size:12px">{{ $empleado->user->name }}</p>
        </div>
    ...
    <div class="content">
        ...
        <div class="form-card">
            <form method="POST" action="{{ route('admin.trabajadores.actualizar', $empleado->id) }}">
                @csrf
                @method('PATCH')
                ...
                <div class="grid-2">
                    <div class="field">
                        ...
                        <input type="text" name="name"
                            value="{{ old('name', $empleado->user->name) }}" required/>
                    </div>
                    <div class="field">
                        <label>Correo electrónico</label>
                        <input type="email" value="{{ $empleado->user->email }}" disabled
                ...
                <div class="grid-2">
                    <div class="field">
                        ...
                        <select name="cargo" required>
                            ...
                            <option value="Soldador" {{ old('cargo', $empleado->cargo) == 'Soldador' ? 'selected' : '' }}>Soldador</option>
                            <option value="Armador" {{ old('cargo', $empleado->cargo) == 'Armador' ? 'selected' : '' }}>Armador</option>
                            <option value="Operador de CNC" {{ old('cargo', $empleado->cargo) == 'Operador de CNC' ? 'selected' : '' }}>Operador de CNC</option>
                            <option value="Ayudante Metal Mecánica" {{ old('cargo', $empleado->cargo) == 'Ayudante Metal Mecánica' ? 'selected' : '' }}>Ayudante Metal Mecánica</option>
                            <option value="Mantenimiento" {{ old('cargo', $empleado->cargo) == 'Mantenimiento' ? 'selected' : '' }}>Mantenimiento</option>
                            <option value="Oficios Varios" {{ old('cargo', $empleado->cargo) == 'Oficios Varios' ? 'selected' : '' }}>Oficios Varios</option>
                            <option value="SST" {{ old('cargo', $empleado->cargo) == 'SST' ? 'selected' : '' }}>SST</option>
                        </select>
                    </div>
                    <div class="field">
