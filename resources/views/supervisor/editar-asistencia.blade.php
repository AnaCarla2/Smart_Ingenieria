...
<head>
    ...
    <title>Smart Ingeniería — Editar Asistencia</title>
    <style>
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .info-box{background:#1c1c1e;border:1px solid #27272a;border-radius:8px;padding:12px 16px;margin-bottom:20px}
...
<div class="main">
    <div class="topbar">
        <div>
            <p style="color:#fff;font-weight:700;font-size:15px">Editar Asistencia</p>
            <p style="color:#71717a;font-size:12px">Corrección manual de registro</p>
        </div>
        <div class="user-menu">
    ...
    <div class="content">
        ...
        <div class="info-box">
            ...
            <p style="color:#fff;font-weight:700;font-size:15px">{{ $asistencia->empleado->user->name }}</p>
            <p style="color:#71717a;font-size:12px">
                {{ $asistencia->empleado->cargo }} ·
                Fecha: {{ \Carbon\Carbon::parse($asistencia->fecha)->format('d/m/Y') }}
            </p>
        </div>

        <div class="form-card">
            <p class="form-title">✏️ Corregir registro de asistencia</p>
            <form method="POST" action="{{ route('supervisor.asistencia.actualizar', $asistencia->id) }}">
                @csrf
                @method('PATCH')
                ...
                <div class="grid-2">
                    <div class="field">
                        ...
                        <input type="time" name="hora_entrada"
                            value="{{ \Carbon\Carbon::parse($asistencia->hora_entrada)->format('H:i') }}"
                            required/>
                    </div>
                    <div class="field">
                ...
                <div class="field">
                    ...
                    <select name="estado" required>
                        ...
                        <option value="permiso" {{ $asistencia->estado === 'permiso' ? 'selected' : '' }}>Permiso</option>
                        <option value="incapacidad" {{ $asistencia->estado === 'incapacidad' ? 'selected' : '' }}>Incapacidad</option>
                    </select>
                </div>
                ...
                <div class="field">
                    ...
                    <textarea name="observacion" rows="2"
                        placeholder="Motivo de la corrección...">{{ $asistencia->observacion }}</textarea>
                </div>

                <div style="display:flex;gap:12px;margin-top:8px">
                    <button type="submit" class="btn btn-amarillo">✅ Guardar corrección</button>
                    <a href="{{ route('supervisor.dashboard') }}" class="btn btn-gris">Cancelar</a>
                </div>
